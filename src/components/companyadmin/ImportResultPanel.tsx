/**
 * File: ImportResultPanel.tsx
 * Description: Final summary shown after POST /users/import/confirm succeeds.
 *              Highlights created/updated counts and surfaces per-row errors.
 */

import { Button } from '../ui/Button';
import { ImportResult } from '../../types/importEmployees';
...
/**
 * Renders the success summary and any row-level errors returned by the backend.
 */
const ImportResultPanel = ({ result, onReset }: ImportResultPanelProps) => {
    ...
    <section className="bg-white border border-gray-200 rounded-xl p-6 space-y-6 shadow-sm">
      <header className="space-y-1">
        ...
        <p className="text-sm text-gray-500">
          Se procesó el archivo y se aplicaron las asignaciones de rol configuradas.
        </p>
      </header>
        ...
        <div className="border border-red-200 rounded-lg overflow-hidden">
          ...
          <ul className="divide-y divide-red-100">
            {result.errors.map((error, index) => (
              <li
                key={`${error.employeeNumber || 'row'}-${index}`}
                className="px-4 py-2 text-sm text-red-700 bg-white"
              >
                <span className="font-semibold">
                  {error.employeeNumber || 'Sin No. Empleado'}:
                </span>{' '}
                {error.message}
              </li>
            ))}
