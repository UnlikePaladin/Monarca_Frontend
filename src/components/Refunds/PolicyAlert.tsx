                ...
                {v.evaluated_value && (
                  <div className="mt-2 text-xs font-medium opacity-90 bg-white/60 p-2 rounded border border-current/10 font-mono">
                    {['LT', 'LTE', 'GT', 'GTE'].includes(v.policy_code) || v.evaluated_value.operator && (
                        <p>Monto ingresado: <span className="font-bold">${v.evaluated_value.amount}</span> | Límite: <span className="font-bold">{v.evaluated_value.operator} {v.evaluated_value.threshold} {v.evaluated_value.currency}</span></p>
                    )}
                    {(v.policy_code === 'TOTAL_LTE_ADVANCE' || v.message.includes('Total vouchers')) && (
                      <p>Total detectado: <span className="font-bold text-red-700">${v.evaluated_value.total_vouchers}</span> | Límite (Anticipo): <span className="font-bold">${v.evaluated_value.advance_money}</span></p>
                    )}
                    ...
                    {(v.policy_code === 'DAYS_EXCEEDED' || v.policy_code === 'TIME_LIMIT') && (
                      <p>Días desde la creación: <span className="font-bold text-red-700">{v.evaluated_value.elapsed_days}</span> | Máximo permitido: <span className="font-bold">{v.evaluated_value.max_days} días</span></p>
                    )}
                  </div>
                )}
              </div>
