...
import type {
  AccountingAccountImportConfirmPayload,
  AccountingAccountImportPreviewResponse,
  AccountingAccountImportPreviewRow,
  AccountingAccountImportResult,
} from '../../types/importAccountingAccounts';

type Step = 'upload' | 'preview' | 'done';
type ApiErrorBody = { message?: string | string[] };

const TEMPLATE_URL = '/templates/plantilla-cuentas-contables.xlsx';
const previewFields = {
  key: 'Clave',
  description: 'Descripción',
  requiresCostCenter: 'Requiere centro de costos',
...
const normalizeRows = (rows: AccountingAccountImportPreviewRow[]) =>
  rows.map((row) => ({
    ...row,
    validationErrors: normalizeValidationMessages(row.validationErrors ?? row.errors),
  }));

const ImportAccountingAccounts = () => {
  const { setPageTitle } = useApp();
  const { authState } = useAuth();
  const companyId = authState.userCompanyId ?? '';

  const [step, setStep] = useState<Step>('upload');
  const [preview, setPreview] = useState<AccountingAccountImportPreviewResponse | null>(null);
  const [result, setResult] = useState<AccountingAccountImportResult | null>(null);

  const previewEndpoint = `/companies/${companyId}/accounting-accounts/import/preview`;
  const confirmEndpoint = `/companies/${companyId}/accounting-accounts/import/confirm`;

  const { mutateAsync: runPreview, isPending: isPreviewing } = useExcelImportPreview<AccountingAccountImportPreviewResponse>(
    previewEndpoint,
  );
  const { mutateAsync: runConfirm, isPending: isConfirming } = useExcelImportConfirm<
    AccountingAccountImportConfirmPayload,
    AccountingAccountImportResult
  >(confirmEndpoint, [['companyAccountingAccounts', companyId]]);

  useEffect(() => {
    setPageTitle('Importar cuentas contables');
  }, [setPageTitle]);
  ...
  const handleUpload = async (file: File) => {
    try {
      const response = normalizePreviewResponse<AccountingAccountImportPreviewRow>(
        await runPreview(file),
        ['accounts', 'rows', 'accountingAccounts', 'data'],
      );
      setPreview(response);
      setStep('preview');
  ...
  const handleConfirm = async () => {
    ...
    const payload: AccountingAccountImportConfirmPayload = {
      accounts: validRows.map((row) => {
        const stripped = stripImportMetadata(row) as any;
        return {
          row: row.row,
          key: stripped.key,
          description: stripped.description,
          requiresCostCenter: stripped.requiresCostCenter,
          idBankAccount: stripped.idBankAccount,
    ...
    return (
      <section className="px-6 md:px-16 pt-32 flex-1 pb-12">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl font-bold text-gray-900">Importar cuentas contables</h1>
  ...
  return (
    <div className="px-6 md:px-16 pt-32 flex-1 pb-12">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {step === 'upload' && (
          <ImportUploadCard
            stepTitle="1. Cargar archivo de cuentas contables"
            description="Descarga la plantilla, completa las cuentas contables y súbela para obtener una vista previa antes de confirmar."
            templateUrl={TEMPLATE_URL}
            columnsHint="Columnas en plantilla: Clave, Descripción, Requiere centro de costos, Cuenta bancaria."
        ...
        {step === 'preview' && preview && (
          <section className="space-y-6">
            ...
            <ImportPreviewTable<AccountingAccountImportPreviewRow>
              rows={rows}
              columns={[
                {
                  header: 'Clave',
                ...
                { header: 'Requiere centro de costos', render: (row) => (row.requiresCostCenter ? 'Sí' : 'No') },
                { header: 'Cuenta bancaria', render: (row) => row.bankAccountName ?? '—' },
              ]}
              emptyMessage="No hay cuentas contables en el archivo."
              getRowKey={(row) => `${row.row}-${row.key}`}
        ...
        {step === 'done' && result && (
          <ImportResultPanel
            ...
            description="Se procesaron las cuentas contables validadas del archivo."
            result={result}
            onReset={handleReset}
          />
        )}
...
export default ImportAccountingAccounts;
