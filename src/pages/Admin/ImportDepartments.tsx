...
import type {
  DepartmentImportConfirmPayload,
  DepartmentImportPreviewResponse,
  DepartmentImportPreviewRow,
  DepartmentImportResult,
} from '../../types/importDepartments';

type Step = 'upload' | 'preview' | 'done';
type ApiErrorBody = { message?: string | string[] };

const TEMPLATE_URL = '/templates/plantilla-departamentos.xlsx';
const previewFields = {
  name: 'Nombre',
...
const normalizeRows = (rows: DepartmentImportPreviewRow[]) =>
  rows.map((row) => ({
    ...row,
...
const ImportDepartments = () => {
  ...
  const [preview, setPreview] = useState<DepartmentImportPreviewResponse | null>(null);
  const [result, setResult] = useState<DepartmentImportResult | null>(null);

  const previewEndpoint = `/companies/${companyId}/departments/import/preview`;
  const confirmEndpoint = `/companies/${companyId}/departments/import/confirm`;

  const { mutateAsync: runPreview, isPending: isPreviewing } = useExcelImportPreview<DepartmentImportPreviewResponse>(
    previewEndpoint,
  );
  const { mutateAsync: runConfirm, isPending: isConfirming } = useExcelImportConfirm<
    DepartmentImportConfirmPayload,
    DepartmentImportResult
  >(confirmEndpoint, [['companyDepartments', companyId]]);

  useEffect(() => {
    setPageTitle('Importar departamentos');
  }, [setPageTitle]);
  ...
  const handleUpload = async (file: File) => {
    try {
      const response = normalizePreviewResponse<DepartmentImportPreviewRow>(
  ...
  const handleConfirm = async () => {
    ...
    const payload: DepartmentImportConfirmPayload = {
      departments: validRows.map((row) => stripImportMetadata(row)),
    };
    ...
    return (
      <section className="px-6 md:px-16 pt-32 flex-1 pb-12">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl font-bold text-gray-900">Importar departamentos</h1>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <header className="space-y-1">
          ...
          <p className="text-sm text-gray-500">
            Sube un archivo Excel, revisa la validación por fila y confirma solo los
        ...
        {step === 'upload' && (
          <ImportUploadCard
            stepTitle="1. Cargar archivo de departamentos"
            description="Descarga la plantilla, completa los departamentos y súbela para obtener una vista previa antes de confirmar."
            templateUrl={TEMPLATE_URL}
            columnsHint="Columnas en plantilla: Nombre, Centro de costos."
        ...
        {step === 'preview' && preview && (
          <section className="space-y-6">
            ...
            <ImportPreviewTable<DepartmentImportPreviewRow>
              rows={rows}
              columns={[
                { header: 'Nombre', render: (row) => row.name ?? '—' },
                {
                  header: 'Centro de costos',
                  render: (row) => row.costCenterName ?? row.cost_center_id ?? '—',
                },
              ]}
              emptyMessage="No hay departamentos en el archivo."
        ...
        {step === 'done' && result && (
          <ImportResultPanel
            ...
            description="Se procesaron los departamentos validados del archivo."
            result={result}
            onReset={handleReset}
          />
        )}
